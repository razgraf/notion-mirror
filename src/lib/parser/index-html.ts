...
function parseUlElement(html: string, startIndex: number): { node: NavNode | null; endIndex: number } {
  // Find the <ul> tag
  ...
  while (currentIndex < html.length) {
    ...
    const aHrefMatch = html.slice(currentIndex).match(/^<a\s+href="([^"]*)"[^>]*>([^<]*)<\/a>/)
    if (aHrefMatch) {
  ...
  const uuid = extractUuid(nodeId) || extractUuid(filePath || '') || extractUuid(title)
...
export function parseIndexHtml(): WorkspaceData {
  ...
  const indexPath = path.join(dataPath, 'index.html')
  ...
  const workspaceName = workspaceNameMatch ? workspaceNameMatch[1].trim() : 'Workspace'
  ...
  let searchIndex = 0
  const workspaceDetailsEnd = html.indexOf('</li>', html.indexOf('Workspace name:'))
...
export function findNodeById(tree: NavNode[], id: string): NavNode | null {
  for (const node of tree) {
    if (node.id === id || node.id.startsWith(id) || id.startsWith(node.id.slice(0, 8))) {
      return node
    }
...
export function getBreadcrumbs(tree: NavNode[], targetId: string): NavNode[] {
  function search(nodes: NavNode[], path: NavNode[]): NavNode[] | null {
    for (const node of nodes) {
      ...
      if (node.id === targetId || node.id.startsWith(targetId) || targetId.startsWith(node.id.slice(0, 8))) {
        return newPath
      }
