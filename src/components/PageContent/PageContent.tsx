...
import { useMemo, useEffect, useState, type ComponentProps } from 'react'
...
function parseDbItemContent(content: string): { title: string; properties: PropertyItem[]; body: string } | null {
  ...
  function search(nodes: NavNode[]): NavNode | null {
    for (const node of nodes) {
      if (node.filePath) {
        ...
        if (normalizedNodePath === normalizedTarget || normalizedNodePath.endsWith(normalizedTarget)) {
          return node
        }
      }
...
export function PageContent({ content, filePath, navTree }: PageContentProps) {
  ...
  const components: Partial<ComponentProps<typeof ReactMarkdown>['components']> = useMemo(
    () => ({
      ...
      a: ({ href, children, ...props }) => {
        ...
        // Broken link
        return <span className="broken-link" title={`Link not found: ${href}`}>{children}</span>
      },
      ...
      code: ({ className, children, ...props }) => {
        ...
        if (highlighter) {
          try {
            const html = (highlighter as { codeToHtml: (code: string, opts: { lang: string; theme: string }) => string }).codeToHtml(code, {
              lang,
              theme: 'github-dark',
            })
        ...
        if (isEmoji) {
          return (
            <div className="callout">
              ...
              <div className="callout-content">{text.slice(emojiMatch[0].length)}</div>
            </div>
          )
        }
